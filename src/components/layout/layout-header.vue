<template>
    <div id="layout-header" :class="{ 'fixed': fixed, 'hidden': hidden }" @click.stop="mobileShow = false">
        <div class="site-logo">
            <router-link to="/">
                <img src="@/assets/site-logo.svg" alt="" style="transform: scale(0.7);">
                <p class="site-name">L - Tao</p>
            </router-link>
        </div>
            ...
            <div class="menu-item hasChild">
                ...
                <div class="childMenu" v-if="category.length">
                    <div class="sub-menu" v-for="item in category" :key="item.title">
                        <div @click="toPush(item)">{{ item.title }}</div>
                    </div>
                </div>
            </div>
...
export default {
    ...
    methods: {
        ...
        fetchCategory() {
            axios.get('/webapi/category').then(res => {
